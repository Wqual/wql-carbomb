...
RegisterNetEvent('wql-carbomb:CheckIfRequirementsAreMet')
AddEventHandler('wql-carbomb:CheckIfRequirementsAreMet', function()
    local ped = GetPlayerPed(-1)
    ...
    if not IsPedInAnyVehicle(ped, false) then
        if veh and (dist < 3.0) then
            ...
            TriggerServerEvent('wql-carbomb:RemoveBombFromInv')
